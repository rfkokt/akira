...
├──────┬────────────────────────────────┬─────────────────────────┬───────────┤
│      │                                │  ┌── File Tabs ───────┐ │           │
│  S   │     GLOBAL CHAT PANEL          │  │ App.tsx │ README.md│ │  SOURCE   │
│  I   │     (Persistent)               │  ├────────────────────┤ │  CONTROL  │
│  D   │                                │  │  EDITOR / PREVIEW  │ │           │
│  E   │     [TaskCreatorChat]          │  │                    │ │  [Msg...] │
│  B   │                                │  │  • .md → Preview   │ │  [Auto]   │
│  A   │     - Visible on Tasks Page    │  │  • .ts → Monaco    │ │  [Commit] │
│  R   │     - Visible on Files Page    │  │                    │ │           │
│      │                                │  │                    │ │  ▾ Changes│
│      │                                │  │                    │ │  M file.ts│
│      │                                │  └────────────────────┘ │           │
│      │                                │                         │  GIT GRAPH│
...
## 💬 Feature 0: Global Persistent Chat (Pre-requisite)

### Konsep UI & Architecture
Memindahkan komponen chat dari dalam `KanbanBoard` ke level `App.tsx` agar chat panel (TaskCreatorChat) selalu terlihat baik saat user berada di halaman **Tasks/Board** maupun di halaman **Files/Editor**.

### Komponen yang Terpengaruh:
1. `App.tsx`:
   - Menambahkan state `showChat` (global toggle).
   - Memasukkan `<TaskCreatorChat>` ke main layout container.
2. `src/components/Kanban/Board.tsx`:
   - Menghapus local state `showTaskCreator`.
   - Menghapus komponen `<TaskCreatorChat>` dari Board layout.

### Data Flow
```
App Layout
 ├─ Sidebar Navigation
 ├─ Global Chat Panel (<TaskCreatorChat>) [Toggable]
 └─ Main View Area
     ├─ Route: 'tasks' → <KanbanBoard /> (Murni board dnd)
     └─ Route: 'files' → FileTree + FileViewer + GitPanel
```
